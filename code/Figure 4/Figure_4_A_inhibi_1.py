...
def multicores(args):
    ...
    # inhibitory_neurons2 = np.zeros(MAX_NUMBER_CONFIGURATIONS, dtype=float)
    ...
    for i in range(0, NUMBER_OF_NEURONS):
        for j in range(0, NUMBER_OF_NEURONS):
            ...
            e_e_connections_pools.append([i, j])
    ...
    deleted_Cons = random.sample(e_e_connections_pools,
                                 round((deleteRate_EtoE / 100) * len(e_e_connections_pools)))

    for a in range(0, round((deleteRate_EtoE / 100) * len(e_e_connections_pools))):
    ...
    for i in range(0, NUMBER_OF_LOOPS):
        ...
        # inhibitory_neurons2[sum_list2.index(max(sum_list2))] = 1
...
######################################################################
capacities = []
folder_name = "50 E-E deficiency/inhibi_1"
if __name__ == '__main__':
    ...
    plt.ylim(-0.2, 1.2)
    # plt.title("Capacity vs Saturation for 50 ex-neurons and 1 in-neuron system")
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_1_in_neuron_E-E_deficiency" + ".pdf", format='pdf')
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_1_in_neuron_E-E_deficiency" + ".svg", format='svg')
    plt.show()