...
def multicores(args):
    ...
    big_connections_pool = i_e_connections_pool1 + i_e_connections_pool2 + i_e_connections_pool3
...
if __name__ == '__main__':
    for NUMBER_OF_TEMPERATURE1 in range(0, MAX_NUMBER_TEMPERATURES1):
        ...
        output_folder = "results_output_IE_deficiency3/"
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)

        output_file = os.path.join(output_folder, "X1_and_average_res_by_def_i_e3.xlsx")
    ...
    plt.ylim(-0.2, 1.2)
    # plt.title("Capacity vs Saturation for 50 ex-neurons and 3 in-neuron system")
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_3_in_neuron_I-E_deficiency_no_inner_comparison" + ".pdf",
                format='pdf')
