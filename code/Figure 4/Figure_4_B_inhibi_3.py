...
def multicores(args):
    ...
    big_connections_pool = e_i_connections_pool1 + e_i_connections_pool2 + e_i_connections_pool3
...
if __name__ == '__main__':
    for NUMBER_OF_TEMPERATURE1 in range(0, MAX_NUMBER_TEMPERATURES1):
        ...
        output_folder = "results_output_EI_deficiency3/"
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)

        output_file = os.path.join(output_folder, "X1_and_average_res_by_def_e_i3.xlsx")
        result_df = pd.DataFrame()

        for NUMBER_OF_TEMPERATURE in range(0, MAX_NUMBER_TEMPERATURES):
            folder_name = "50 E-I deficiency/"
            #
            ...
            for def_e_i in [0, 25, 50, 75]:
                ...
                result_df[f"X1_def_e_i_{def_e_i}"] = X1[NUMBER_OF_TEMPERATURE1]
                result_df[f"average_res_def_e_i_{def_e_i}"] = average_res
    ...
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_3_in_neuron_E-I_deficiency_no_inner_comparison" + ".pdf",
                format='pdf')
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_3"
                            ""
                            "_in_neuron_E-I_deficiency_no_inner_comparison" + ".svg",
                format='svg')
    plt.show()