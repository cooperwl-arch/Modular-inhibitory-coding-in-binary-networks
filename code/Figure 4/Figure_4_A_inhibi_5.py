...
def save_capacity_metadata(raw_data_folder):
    metadata = {
        ...
        "separate_csv_files": [
            "raw_data_def_e_e_0.csv",
            "raw_data_def_e_e_25.csv",
            "raw_data_def_e_e_50.csv",
            "raw_data_def_e_e_75.csv"
        ],
        "parameter_varied_between_files": {
            "fraction of removed E-E connections (%)": [0, 25, 50, 75]
        },
...
def save_capacity_raw_data(res, x, def_e_e, folder_name):
    ...
    df.to_csv(
        os.path.join(
            raw_data_folder,
            f"raw_data_def_e_e_{def_e_e}.csv"
        )
    )
...
def multicores(args):
    ...
    i_e_connections_pool1 = []
    i_e_connections_pool2 = []
    i_e_connections_pool3 = []
    i_e_connections_pool4 = []
    i_e_connections_pool5 = []
    ...
    for i in range(0, NUMBER_OF_NEURONS):
        ...
        for k in range(0, NUMBER_OF_CONFIGURATIONS):
            ...
            if Configurations[k][i] == 1:
                ...
                i_e_connections_pool1.append([1, k, i])
                i_e_connections_pool2.append([2, k, i])
                i_e_connections_pool3.append([3, k, i])
                i_e_connections_pool4.append([4, k, i])
                i_e_connections_pool5.append([5, k, i])


    big_connections_pool = i_e_connections_pool1 + i_e_connections_pool2 + i_e_connections_pool3 + i_e_connections_pool4 + i_e_connections_pool5

    deleted_Cons = random.sample(big_connections_pool,
                                 round((deleteRate_ItoE / 100) * len(big_connections_pool)))
    random.shuffle(deleted_Cons)

    ######################### delete connections #################################
    for a in range(0, round((deleteRate_ItoE / 100) * len(big_connections_pool))):
        if (deleted_Cons[a])[0] == 1:
            ...
            index1 = (deleted_Cons[a])[1]
            index2 = (deleted_Cons[a])[2]
            inhibitory_to_excitatory_connections2[index1][index2] = 0
        elif (deleted_Cons[a])[0] == 3:
            index1 = (deleted_Cons[a])[1]
            index2 = (deleted_Cons[a])[2]
            inhibitory_to_excitatory_connections3[index1][index2] = 0
        elif (deleted_Cons[a])[0] == 4:
            index1 = (deleted_Cons[a])[1]
            index2 = (deleted_Cons[a])[2]
            inhibitory_to_excitatory_connections4[index1][index2] = 0
        elif (deleted_Cons[a])[0] == 5:
            index1 = (deleted_Cons[a])[1]
            index2 = (deleted_Cons[a])[2]
            inhibitory_to_excitatory_connections5[index1][index2] = 0
    ############################################################
...
######################################################################
capacities = []
folder_name = "50 E-E deficiency/inhibi_5"
if __name__ == '__main__':
    for NUMBER_OF_TEMPERATURE1 in range(0, MAX_NUMBER_TEMPERATURES1):
        ...
        lower_value = []
        t = 0
        output_folder = "results_output_IE_deficiency5/"
        if not os.path.exists(output_folder):
            os.makedirs(output_folder)

        output_file = os.path.join(output_folder, "X1_and_average_res_by_def_i_e5.xlsx")
        ...
        for NUMBER_OF_TEMPERATURE in range(0, MAX_NUMBER_TEMPERATURES):
            ...
            for def_e_e in [0, 25, 50, 75]:
                ...
                x = np.arange(start, MAX_NUMBER_CONFIGURATIONS + 1, 50)
                save_capacity_raw_data(res, x, def_e_e, folder_name)
    ...
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_5_in_neuron_E-E_deficiency" + ".pdf",
                format='pdf')
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_5_in_neuron_E-E_deficiency"+ ".svg",
                format='svg')
    plt.show()