    ...
    for i in range(0, NUMBER_OF_NEURONS):
        ...
        for k in range(0, NUMBER_OF_CONFIGURATIONS):
            ...
            if Configurations[k][i] == 1:
                ...
                e_i_connections_pools.append([i, k])

            else:
                inhibitory_to_excitatory_connections[k][i] = -1

    deleted_Cons = random.sample(e_i_connections_pools,
                                 round((deleteRate_EtoI / 100) * len(e_i_connections_pools)))

    for a in range(0, round((deleteRate_EtoI / 100) * len(e_i_connections_pools))):
...
######################################################################
capacities = []
folder_name = "50 E-I deficiency/inhibi_1"
if __name__ == '__main__':
    ...
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_1_in_neuron_E-I_deficiency" + ".pdf", format='pdf')
    plt.savefig(file_name + "/capacity_50_ex-neurons_system_1_in_neuron_E-I_deficiency" + ".svg", format='svg')
    plt.show()